...
function VenderReg() {
    ...
    useEffect(()=>{
        axios.get('http://localhost:9995/venders/getvendercount/').then((res)=>{
    ...
    const handleRegistrationButton=()=>{
        ...
        axios.post('http://localhost:9995/venders/register/',obj).then((res)=>{
            alert(res.data);
        }).catch((err)=>{
            alert(err);
        })
    ...
    const handleSubmit=async (e)=>{
        ...
        const response=await fetch("http://localhost:9995/venders/savevenderimage",{
            method:"POST",
            body:formData,
        })
        if(response){
...
<table>
    ...
    <tr>
        <td>Vendor Name</td>
        <td>
            <input type='text' onChange={handleVenderName}/>
        </td>
    </tr>
    <tr>
        <td>
