...
function Bill(props)
{
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:9995/customer/getcustomerdetails/"+props.data.cid).then((res)=>{
    ...
    {
        alert(sitems.length);
        axios.get('http://localhost:9995/bill/getbillid').then((res)=>{
       ...
       sitems.map((item)=>{
        ...
        axios.post('http://localhost:9995/bill/billsave',billobj).then((res)=>{
            alert(res.data);
        })
        .catch((err)=>{
            alert("inner"+err);
    ...
    async function displayRazorPay() {
        ...
        //creating new order
        const result=await axios.post("http://localhost:9995/payment/orders/"+myamount);
        if(!result){
        ...
        const options={
            ...
            handler:async function (response) {
                ...
                const result=await axios.post("http://localhost:9995/payment/success",data);
                alert(result.data);
                ...
                axios.post("http://localhost:9995/paymentdetails/paymentdetailsave",paydetlobjdata).then((res)=>{
                    alert(res.data);
                ...
                </tr>
                {
                    props.data.selitems.map((item)=>(
                        <tr>
                            ...
                            <td>{item.pname}</td>
                            <td>{item.oprice}</td>
                            <img src={"http://localhost:9995/product/getproductimage/"+item.ppicname}/>
                        </tr>
                    ))
                }
            </table>
