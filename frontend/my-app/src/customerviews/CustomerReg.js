...
function CustomerReg()
{
    ...
    const handleStIdSelect=(e)=>{
        ...
        axios.get('http://localhost:9995/city/getcitybystid/'+e.target.value).then((res)=>{
            setCtList(res.data);
        }).catch((err)=>{
            alert(err);
        })
    ...
    useEffect(()=>{
        axios.get('http://localhost:9995/customer/getcustomercount/').then((res)=>{
    ...
    const handleRegisterButton= async ()=>{
             ...
             axios.post('http://localhost:9995/customer/register/',obj).then((res)=>{
                alert(res.data);
             }).catch((err)=>{
                alert(err);
             })
            ...
            const handleSubmitButton=async(e)=>{
             ...
             const response=await fetch('http://localhost:9995/customer/savecustomerimage',{
                method:"POST",
                body:formData,
             });
             if(response)
