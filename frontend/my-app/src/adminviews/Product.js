...
function Product(props) {
    ...
    useEffect(() => {
        axios.get('http://localhost:9995/product/getmaxpid').then((res) => {
            setpid(res.data.length + 1);

        }).catch((err) => {
            alert(err);
        });
        axios.get('http://localhost:9995/productcatg/showproductcatg').then(res => {
    ...
    const handleSaveButton = () => {
        ...
        axios.post('http://localhost:9995/product/saveproduct/', obj).then((res) => {
            alert("Product Saved");
        }).catch((err) => {
            alert(err);
        })
    ...
    const handleShowButton = () => {
        axios.get('http://localhost:9995/product/showproduct/').then((res) => {
            setPlist(res.data);
    ...
    const handleSubmit = async (evt) => {
        ...
        const response = await fetch('http://localhost:9995/product/saveproductimage', {
            method: 'POST',
            body: formData,
        })
        if(response)
