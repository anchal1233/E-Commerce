...
import cart from '../adminviews/cart1.jpg';
...
function ProductList(props) {
    ...
    const handleSearch = (evt) => {
        if (evt.target.value > 0) {
            axios.get("http://localhost:9995/product/showproductbycatgid/" + evt.target.value).then((res) => {
                setPList(res.data);
            }).catch((err) => {
                alert(err);
            });
                ...
                </tr>
                {
                    plist.map((item) => (
                        <tr>
                            ...
                            <td>
                                <img src={"http://localhost:9995/product/getproductimage/" + item.ppicname} height="100" width="100" />
                            </td>
                            <td>
